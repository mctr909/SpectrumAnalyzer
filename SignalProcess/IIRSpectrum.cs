...
public class IIRSpectrum : BaseSpectrum {
	...
	public double BandWidthMax { get; set; } = 5.0;
	/// <summary>バンド幅の下限[半音]</summary>
	public double BandWidthFloor { get; set; } = 0.2;
	/// <summary>バンド幅の変化の急峻さ[オクターブ]</summary>
	public double BandWidthTransition { get; set; } = 7.0;
	...
	/// <inheritdoc/>
	protected unsafe override void Calc(float* pInput, int sampleCount) {
		var pWaveTerm = pInput + sampleCount*2;
		var banks = Banks;
		/* デノーマル対策 */
		...
		for (int i = 0; i < BankCount; i++) {
			...
			float a0, b0;
			for (var pWave = pInput; pWave < pWaveTerm; pWave+=2) {
				/* IIR BPF(左) */
				b0 = pWave[0];
				a0 = b0 * kb0;
				a0 -= lb2 * kb0;
