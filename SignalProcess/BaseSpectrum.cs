...
public abstract class BaseSpectrum {
	...
	private const float NEAR_AVG_GAIN = 1f;
	...
	/// <param name="pInput">入力バッファ(2ch×サンプル数)</param>
	/// <param name="sampleCount">入力バッファのサンプル数</param>
	protected unsafe abstract void Calc(float* pInput, int sampleCount);
		...
		for (int ib = 0; ib < BankCount; ib++) {
			...
			double avgL, avgR;
			{
				...
				for (int iw = iwStart; iw <= iwEnd; iw++) {
					var t = (iw - ib) * (float)Math.E / NEAR_AVG_RADIUS;
			...
			isPeakL &= centerL >= avgL;
			isPeakR &= centerR >= avgR;

			/* リニア化 */
			var threshold = (float)Math.Sqrt(Math.Max(thresholdL, thresholdR) * 2);
			var avg = (float)Math.Sqrt(Math.Max(avgL, avgR) * 2);
