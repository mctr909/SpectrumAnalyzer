...
public class OscBank {
	...
	const double DECLICK_SPEED = 0.04;
	...
	public unsafe void SetWave(Spectrum spectrum, IntPtr pOutput) {
		...
		for (int idxT = 0, idxB = 0; idxT < Tones.Length; idxT++, idxB += Spectrum.TONE_DIV) {
			...
			var specAmpR = 0.0;
			var specAmpC = 0.0;
			var delta = spectrum.Banks[idxB + Spectrum.TONE_DIV_CENTER].PeakDelta;
			for (int div = 0, divB = idxB; div < Spectrum.TONE_DIV; div++, divB++) {
				var peakL = spectrum.PeakL[divB];
				var peakR = spectrum.PeakR[divB];
				...
				if (specAmpC < peakC) {
					specAmpC = peakC;
					delta = spectrum.Banks[divB].PeakDelta;
				}
			}
			specAmpL *= spectrum.GainL * 1.41;
			specAmpR *= spectrum.GainR * 1.41;
			var tone = Tones[idxT];
