...
namespace Spectrum {
	public partial class Spectrum {
		...
		const int BEGIN_HIGH = HALFTONE_DIV * 54;
		/// <summary>低音域 閾値幅[フィルタバンク数]</summary>
		const int THRESHOLD_WIDTH_LOW = HALFTONE_DIV * 7;
		/// <summary>高音域 閾値幅[フィルタバンク数]</summary>
		const int THRESHOLD_WIDTH_HIGH = HALFTONE_DIV * 3 / 4;
		/// <summary>低音域 閾値ゲイン</summary>
		const double THRESHOLD_GAIN_LOW = 1.122;
		/// <summary>高音域 閾値ゲイン</summary>
		const double THRESHOLD_GAIN_HIGH = 1.122;
	}
}
