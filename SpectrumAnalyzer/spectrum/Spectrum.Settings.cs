...
namespace Spectrum {
	public partial class Spectrum {
		...
		/// <summary>平均値に対するゲイン</summary>
		public static readonly double AVG_GAIN = Math.Sqrt(Math.Pow(10, 0.1/20.0));
		...
		/// <summary>帯域幅が1半音に至る周波数[Hz]</summary>
		const double HALFTONE_AT_FREQ = 300.0;

		/// <summary>中音域 開始位置[フィルタバンク数]</summary>
		const int BEGIN_MID = HALFTONE_DIV * 24;
		/// <summary>高音域 開始位置[フィルタバンク数]</summary>
		const int BEGIN_HIGH = HALFTONE_DIV * 72;
		/// <summary>低音域 閾値半径[フィルタバンク数]</summary>
		const int THRESHOLD_RADIUS_LOW = 4;
		/// <summary>高音域 閾値半径[フィルタバンク数]</summary>
		const int THRESHOLD_RADIUS_HIGH = 1;
		/// <summary>平均半径[フィルタバンク数]</summary>
		const int AVG_RADIUS = HALFTONE_DIV * 3/2;
	}
}
