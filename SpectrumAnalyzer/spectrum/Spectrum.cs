...
namespace Spectrum {
	public class Spectrum : IDisposable {
		/// <summary>基本周波数(C0)</summary>
		public const double BASE_FREQ = 16.352; //C0 = 440 * Math.Pow(2.0, 3 / 12.0 - 5)
		...
		const double DISP_SPEED = 60;
		/// <summary>フィルタ遮断幅が1半音に至る周波数[Hz]</summary>
		const double FREQ_AT_HALFTONE_WIDTH = 440.0;

		/// <summary>低音域 終了バンク</summary>
		const int END_LOW_BANK = HALFTONE_DIV * 28;
		/// <summary>低音域 閾値幅[フィルタバンク数]</summary>
		const int THRESHOLD_WIDTH_LOW = HALFTONE_DIV * 4;
		/// <summary>低音域 閾値ゲイン[10^(db/20)]</summary>
		const double THRESHOLD_GAIN_LOW = 1.122;
		/// <summary>中高音域 開始バンク</summary>
		const int BEGIN_MID_BANK = HALFTONE_DIV * 50;
		/// <summary>中高音域 閾値幅[フィルタバンク数]</summary>
		const int THRESHOLD_WIDTH_MID = HALFTONE_DIV;
		...
		public Spectrum(int sampleRate) {
			...
			for (int b = 0; b < BANK_COUNT; ++b) {
				...
				SetBPFParameter(mpFilterBanks[b], SampleRate, frequency);
				SetSigma(mpFilterBanks[b], SampleRate, frequency);
			}
		}
		...
		static unsafe void SetBPFParameter(IntPtr pFilterBank, int sampleRate, double frequency) {
			var omega = 2 * Math.PI * frequency / sampleRate;
			var alpha = GetAlpha(sampleRate, frequency);
			var a0 = 1.0 + alpha;
			var pBank = (FilterBank*)pFilterBank;
		...
		static unsafe void SetSigma(IntPtr pFilterBank, int sampleRate, double frequency) {
			var sampleOmega = sampleRate / (2 * Math.PI);
			var pBank = (FilterBank*)pFilterBank;
			pBank->SIGMA = GetAlpha(sampleOmega, frequency);
			pBank->SIGMA_DISP = GetAlpha(sampleOmega, (DISP_SPEED > frequency) ? frequency : DISP_SPEED);
		}
		...
		unsafe void ExtractPeak() {
			...
			for (int idxB = 0; idxB < BANK_COUNT; ++idxB) {
				...
				var thresholdRDisp = 0.0;
				{
					...
					thresholdL = Math.Sqrt(thresholdL) * gain;
					thresholdR = Math.Sqrt(thresholdR) * gain;
					thresholdLDisp = Math.Sqrt(thresholdLDisp) * gain;
					thresholdRDisp = Math.Sqrt(thresholdRDisp) * gain;
				}
				...
				/*** 表示用のピークを抽出、曲線を設定 ***/
				{
					var linear = Math.Sqrt(Math.Max(bank.LPowerDisp, bank.RPowerDisp));
