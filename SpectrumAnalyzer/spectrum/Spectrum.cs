...
namespace Spectrum {
	public partial class Spectrum : IDisposable {
		...
		private unsafe static void SetBPFCoef(BPF_BANK* p_bank, int sample_rate, double frequency) {
			...
			p_bank->delta = (float)(1.5 * frequency / sample_rate);
		}
		...
		private unsafe void ExtractPeak() {
			...
			for (int ixB = 0; ixB < BANK_COUNT; ++ixB) {
				...
				var threshold_r = 0.0;
				{
					...
					octrms_l /= width;
					octrms_r /= width;
					threshold_l = Math.Max(threshold_l, octrms_l);
					threshold_r = Math.Max(threshold_r, octrms_r);
					/* 2乗平均を振幅に変換 */
					threshold_l = Math.Sqrt(2 * threshold_l);
					threshold_r = Math.Sqrt(2 * threshold_r);
				}
