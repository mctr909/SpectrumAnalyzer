...
namespace SpectrumAnalyzer {
	class Drawer {
		...
		private static readonly Pen PEAK = new Pen(Color.FromArgb(147, 211, 211), 1.0f);
		...
		private readonly double[] mPeakT = new double[BANK_COUNT];
		...
		public void Update(Spectrum.Spectrum spectrum) {
			...
			Array.Copy(mData, mCurve, BANK_COUNT);
			Array.Copy(mData, BANK_COUNT, mThreshold, 0, BANK_COUNT);
			Array.Copy(mData, BANK_COUNT * 2, mPeak, 0, BANK_COUNT);
			Array.Copy(mData, BANK_COUNT * 3, mPeakT, 0, BANK_COUNT);
			...
			if (Forms.Settings.DisplayPeak) {
				Peak(mPeak, gain);
				HueScroll(mPeakT, gain, Forms.Settings.ScrollSpeed);
			} else {
