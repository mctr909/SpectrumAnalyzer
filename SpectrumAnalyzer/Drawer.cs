...
namespace SpectrumAnalyzer {
	static class Drawer {
		...
		static readonly Brush SURFACE_H = new Pen(Color.FromArgb(71, 0, 255, 255)).Brush;
		static readonly Brush LEVEL = new Pen(Color.FromArgb(95, 0, 255, 255)).Brush;
		...
		public static void Level(Graphics g, double val, int height) {
			...
			g.FillRectangle(LEVEL, 10, py, DB_LABEL_WIDTH - 20, barHeight);
		}
		...
		public static void Peak(Graphics g, double[] arr, int width, int height) {
			...
			for (int i = 0; i < BANK_COUNT; i++) {
				...
				if (value > minValue) {
					...
					x0 += ofsX;
					g.FillEllipse(PEAK_TOP, x0, y, barWidth, barWidth);
				}
			}
		}
