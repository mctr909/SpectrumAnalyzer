...
namespace SpectrumAnalyzer {
	static class Drawer {
		...
		static readonly Pen GRID_MAJOR = new Pen(Color.FromArgb(147, 127, 0), 1.0f);
		...
		public static void Gauge(Graphics g, int width, int height) {
			...
			for (var db = dbOfs; dbMin <= db; --db) {
				...
				if (db % 6 == 0) {
					...
					g.DrawLine(GRID_MINOR, DB_LABEL_WIDTH, py, right, py);
				}
			}
		...
		public static void Curve(Graphics g, double[] arr, int width, int height, Pen color) {
			...
			var ofsX = DB_LABEL_WIDTH - HALFTONE_CENTER * BANK_COUNT / width;
			var idxA = 0;
			var px0 = ofsX;
			var py0 = AmpToY(arr[idxA] * scale, height);
			for (int x = 0; x < width; x++) {
				var idxB = x * BANK_COUNT / width;
				var px1 = x + ofsX;
				int py1;
				if (1 < idxB - idxA) {
					py1 = AmpToY(arr[idxA] * scale, height);
					...
					py1 = AmpToY(arr[idxB] * scale, height);
				}
				else {
					py1 = AmpToY(arr[idxB] * scale, height);
					g.DrawLine(color, px0, py0, px1, py1);
		...
		public static void Peak(Graphics g, double[] arr, int width, int height) {
			...
			for (int x = 0, i = 0; x < BANK_COUNT; x++, i++) {
				...
				if (val > minValue) {
					...
					var barHeight = height - py;
					px0 += DB_LABEL_WIDTH;
					g.FillRectangle(PEAK, px0, py, barWidth, barHeight);
				}
			}
		}
			...
			if (DisplayPeak) {
				...
				for (int x = 0, i = 0; x < BANK_COUNT; x++, i++) {
					if (arr[i] * scale > minValue) {
						...
						px0 += DB_LABEL_WIDTH;
						SetHue(arr[i] * scale, offsetY0 + px0 * 4, hueWidth);
					}
				}
			}
			else {
