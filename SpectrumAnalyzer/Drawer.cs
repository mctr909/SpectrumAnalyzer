...
namespace SpectrumAnalyzer {
	class Drawer : IDisposable {
		...
		private static readonly Pen PCurve = new Pen(Color.FromArgb(0, 241, 0), 1.0f);
		private static readonly Pen PThreshold = new Pen(Color.FromArgb(255, 0, 0), 1.0f);
		private static readonly Pen PPeak = new Pen(Color.FromArgb(0, 221, 221), 1.0f);
		...
		#region 公開設定値
		public static int CanpasWidthMin => LabelWidth + BankCount;
		...
		public static int DisplayRangeDb { get; set; } = -24;
		...
		private readonly Fft.Complex[] Fft;
		private readonly uint[] HueValue = new uint[BankCount];
		...
		public Drawer(PictureBox pictureBox) {
			...
			var fftSize = (int)Math.Pow(2, (int)Math.Ceiling(Math.Log(BankCount, 2)) + 1);
			Fft = new Fft.Complex[fftSize];
		}
		...
		public void Update(BaseSpectrum spectrum) {
			...
			if (EnableAutoGain) {
				DrawLevel(spectrum.SmoothedMax, BAutogain);
			}
			if (EnableNormalize) {
		...
		private void DrawSurface(double[] arr, Brush color) {
			...
			GpPoints[BankCount].Y = GraphHeight;
			GpPoints[BankCount + 1].X = GraphLeft;
			GpPoints[BankCount + 1].Y = GraphHeight;
			Gp.Reset();
		...
		private static void Interp(double[] dbArray, int dataLen, double dbMin = -100, double dbRange = 100, double smooth = 0.33) {
			double curent = 0;
			double delta;
