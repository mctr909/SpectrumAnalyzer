...
namespace SpectrumAnalyzer {
	static class Drawer {
		...
		public static void Slope(Graphics g, double[] arr, int count, int offsetIndex, int width, int height, Pen color) {
			var minValue = Math.Pow(10, MinLevel / 20.0);
			for (int x = 0, i = offsetIndex; x < count; x++, i++) {
				var val = arr[i];
				if (val > minValue) {
					...
					g.FillRectangle(color.Brush, barX, barY, barWidth, barHeight);
				}
			}
		}

		public static void Curve(Graphics g, double[] arr, int count, int offsetIndex, int width, int height, Pen color) {
			var idxA = offsetIndex;
			var preX = 0;
			var preY = AmpToY(arr[idxA], height);
			for (int x = 0; x < width; x++) {
				var idxB = offsetIndex + x * count / width;
				int y;
				if (1 < idxB - idxA) {
		...
		public static void Peak(Graphics g, double[] arr, int count, int offsetIndex, int width, int height, Pen color) {
			var minValue = Math.Pow(10, MinLevel / 20.0);
			for (int x = 0, i = offsetIndex; x < count; x++, i++) {
				var val = arr[i];
				if (val > minValue) {
					var barA = x * width / count + 1;
					var barY = AmpToY(val, height);
					var barHeight = height - barY;
					g.FillRectangle(color.Brush, barA, barY, 1, barHeight);
				}
			}
		}

		public static void Scroll(Bitmap bmp, double[] arr, int count, int offsetIndex, int top, int scrollHeight) {
			var width = bmp.Width;
			...
			if (DisplayPeak) {
				for (int x = 0, i = offsetIndex; x < count; x++, i++) {
					if (arr[i] > minValue) {
						var barA = (x - Spectrum.TONE_DIV_CENTER) * width / count;
						var barB = (x + Spectrum.TONE_DIV_CENTER) * width / count;
						SetHue(arr[i], offsetY0 + barA * 4, barB - barA + 2);
					}
				}
			} else {
				for (int x = 0, i = offsetIndex; x < count; x++, i++) {
					if (arr[i] > minValue) {
						...
						SetHue(arr[i], offsetY0 + barA * 4, barB - barA);
					}
				}
			}
