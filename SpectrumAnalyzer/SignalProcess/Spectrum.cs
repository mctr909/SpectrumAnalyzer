...
namespace SignalProcess {
	public class Spectrum : IDisposable {
		...
		public double AvgGain { get; set; } = Math.Pow(10, 0.05 / 10.0);
		#endregion
		...
		public unsafe void SetupFilter(bool initialize) {
			...
			for (int ix = 0; ix < BANK_COUNT; ++ix) {
				...
				/* 中心周波数によってバンド幅を変える */
				var bandWidthOct = ix / (2.7 * OCT_DIV);
				bandWidthOct = 0.5 + 7.5 * Math.Exp(-bandWidthOct * bandWidthOct);
				var bandWidth = bandWidthOct / 12.0;
				/* 正規化周波数 */
		...
		private unsafe void ExtractPeak() {
			...
			for (int ix = 0; ix < BANK_COUNT; ++ix) {
				...
				var avgL = 0.0;
				var avgR = 0.0;
				var ixStart = Math.Max(ix - HALFTONE_DIV, 0);
				var ixEnd = Math.Min(ix + HALFTONE_DIV, BANK_COUNT - 1);
				for (int i = ixStart; i <= ixEnd; ++i) {
					pBank = mpBanks + i;
					avgL += pBank->PowerL;
					avgR += pBank->PowerR;
				}
				var width = HALFTONE_DIV * 2 + 1;
				avgL *= AvgGain / width;
				avgR *= AvgGain / width;
				/* 平均値と最大値をもとに閾値を取得 */
