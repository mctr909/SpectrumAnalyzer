	...
	public class Spectrum : IDisposable {
		...
		public unsafe void SetupFilter(bool initialize) {
			...
			for (int ix = 0; ix < BANK_COUNT; ++ix) {
				...
				/* 中心周波数によってバンド幅を変える */
				var bandWidthOct = ix / (3.5 * OCT_DIV);
				bandWidthOct = 0.33 + 6.67 * Math.Exp(-bandWidthOct * bandWidthOct);
				...
				/* RMSの応答速度を設定 */
				var rmsK = Math.Max(0, (BandWidthHalftoneAtFreq - f0) / SampleRate);
				rmsK = Math.Exp(-rmsK * rmsK);
				bank.RmsSpeed = (float)(fn * rmsK + (1.0 - Math.Exp(-2.0 * fn)) * (1.0 - rmsK));
