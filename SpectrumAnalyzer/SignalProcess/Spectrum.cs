	...
	public class Spectrum : IDisposable {
		...
		private unsafe void ExtractPeak() {
			...
			for (ix = 0; ix < BANK_COUNT; ++ix) {
				/* 局所ピークと最大値を取得 */
				ixStart = Math.Max(ix - 1, 0);
				ixEnd = Math.Min(ix + 1, BANK_COUNT - 1);
				pBank = mpBanks + ix;
				ampL = pBank->PowerL;
				ampR = pBank->PowerR;
				pBank = mpBanks + ixStart;
				isPeakL = ampL > pBank->PowerL;
				isPeakR = ampR > pBank->PowerR;
				maxL = Math.Max(ampL, pBank->PowerL);
				maxR = Math.Max(ampR, pBank->PowerR);
				pBank = mpBanks + ixEnd;
				isPeakL &= ampL > pBank->PowerL;
				isPeakR &= ampR > pBank->PowerR;
				...
				ampL = 0;
				ampR = 0;
				ixStart = Math.Max(ix - AvgRadius, 0);
				ixEnd = Math.Min(ix + AvgRadius, BANK_COUNT - 1);
				...
				ampL = pBank->PowerL;
				ampR = pBank->PowerR;
				isPeakL &= ampL >= thresholdL;
				isPeakR &= ampR >= thresholdR;
				ampL = Math.Sqrt(ampL * 2.0);
				ampR = Math.Sqrt(ampR * 2.0);
				pBank->PeakL = isPeakL ? (float)ampL : 0;
				pBank->PeakR = isPeakR ? (float)ampR : 0;
				/* 表示用の曲線/閾値/ピークを設定 */
				...
				DisplayData[ix + BANK_COUNT * 2] = isPeakL ? ampL : 0;
			}
		}
