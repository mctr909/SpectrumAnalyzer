...
namespace SpectrumAnalyzer {
	public partial class Settings : Form {
		...
		private void Settings_Load(object sender, EventArgs e) {
			...
			TrkThresholdWidth.Value = Spectrum.ThresholdWidth / Spectrum.TONE_DIV;
		...
		private void TrkThresholdWidth_Scroll(object sender, EventArgs e) {
			Spectrum.ThresholdWidth = TrkThresholdWidth.Value * Spectrum.TONE_DIV;
			setting();
		}
		...
		void setting() {
			...
			GrbThreshold.Text = string.Format("閾値幅:{0}半音",　TrkThresholdWidth.Value * 2 + 1);
			GrbMinLevel.Text = string.Format("表示　範囲:{0}db", TrkMinLevel.Value);
		}
	}
}
