...
namespace SpectrumAnalyzer {
	public partial class Settings : Form {
		...
		public const int NOTE_COUNT = 137;
		public static readonly double BASE_FREQ = 220 * Math.Pow(2.0, 3 / 12.0 - 5);
		...
		public static void Open(MainForm parent) {
			...
			mInstance.TrkKey.Value = (int)(Math.Log(OscBank.Pitch * parent.Playback.Speed, 2.0) * 120);
			...
			mInstance.TrkResponce.Value = (int)(0.5 + 2 * Math.Log(Spectrum.ResponceSpeed / 5, 2));
			mInstance.ChkCurve.Checked = Drawer.DisplaySlope;
		...
		private void ChkCurve_CheckedChanged(object sender, EventArgs e) {
			Drawer.DisplaySlope = ChkCurve.Checked;
		}
		...
		private void TrkResponce_Scroll(object sender, EventArgs e) {
			...
			Spectrum.ResponceSpeed = 5 * Math.Pow(2, 0.5 * TrkResponce.Value);
		...
		void SetKeySpeed() {
			Spectrum.Transpose = -0.1 * TrkSpeed.Value / Spectrum.TONE_DIV;
			mParent.Playback.Speed = Math.Pow(2.0, TrkSpeed.Value / (120.0 * Spectrum.TONE_DIV));
			OscBank.Pitch = Math.Pow(2.0, TrkKey.Value / 120.0) / mParent.Playback.Speed;
			var shift = TrkKey.Value / 10.0 - 0.1 * TrkSpeed.Value / Spectrum.TONE_DIV;
		...
		void DispValue() {
			...
			GrbResponce.Text = string.Format("応答速度:{0}Hz", (Spectrum.ResponceSpeed / 4.0).ToString("g3"));
