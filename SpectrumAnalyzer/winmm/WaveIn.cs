...
namespace WinMM {
	public abstract class WaveIn : Wave {
		...
		protected override bool InitializeTask() {
			...
			if (!DeviceEnabled) {
				Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Device Open error:{res}");
				return false;
			}
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Device Opened");
			...
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Header Prepared");
		...
		protected override void FinalizeTask() {
			...
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Reset Device");
			foreach (var pHeader in WaveHeaders) {
				waveInUnprepareHeader(DeviceHandle, pHeader, Marshal.SizeOf<WAVEHDR>());
			}
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Header Unprepared");
			waveInClose(DeviceHandle);
			for (int i = 0; i < 40 && DeviceEnabled; ++i) {
				Thread.Sleep(50);
			}
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Device Closed");
		}
		...
		protected override void Task() {
			...
			Console.WriteLine($"[WaveIn:{Thread.CurrentThread.ManagedThreadId}] Task Stopped");
		}
