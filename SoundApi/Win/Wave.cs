...
public abstract class Wave : IDisposable {
	...
	public delegate void DNotify(ENotify type);
	public DNotify Notify;
	...
	protected readonly int UnitSamples;
	protected readonly int UnitSize;
	protected readonly int UnitCount;
	...
	protected volatile bool Muted = true;
	...
	protected Wave(int sampleRate, double unitTime, int unitCount, DNotify notify) {
		var bytesPerFrame = Bits * Channels >> 3;
		SampleRate = sampleRate;
		DeviceId = DefaultDevice;
		UnitSamples = (int)(sampleRate * unitTime);
		UnitSize = bytesPerFrame * UnitSamples;
		UnitCount = unitCount;
		BufferSamples = UnitSamples * UnitCount;
		BufferSize = bytesPerFrame * BufferSamples;
