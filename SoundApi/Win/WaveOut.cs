...
public abstract class WaveOut : Wave {
	...
	private readonly DWaveOutProc MMCallback;
	...
	public WaveOut(int sampleRate, double unitTime, int unitCount, DNotify notify) : base(sampleRate, unitTime, unitCount, notify) {
		MMCallback = (hwo, uMsg, dwInstance, dwParam1, dwParam2) => {
			switch (uMsg) {
			case MM_WOM.OPEN:
				DeviceOpened.Set();
				break;
	...
	public static List<WaveOutCaps> GetDeviceList() {
		...
		for (int i = -1; i < deviceCount; i++) {
			...
			waveOutGetDevCaps((uint)i, ref caps, Marshal.SizeOf(caps));
			list.Add(caps);
		}
		return list;
	}
	...
	public string GetDeviceName() {
		var caps = new WaveOutCaps();
		var res = waveOutGetDevCaps(DeviceId, ref caps, Marshal.SizeOf(caps));
		if (res == MMSysErr.NOERROR) {
			return caps.szPname;
		} else {
			Console.WriteLine($"[WaveOut] Get Device Name error:{res}");
			return string.Empty;
		}
	}
	...
	protected override bool InitializeTask() {
		var res = waveOutOpen(ref DeviceHandle, DeviceId, ref WaveFormatEx, MMCallback, IntPtr.Zero, SoundApi.Win.MMCallback.FUNCTION);
